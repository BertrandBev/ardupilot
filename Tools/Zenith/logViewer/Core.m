    ...
    methods
        ...
        function init(obj)
            ...
            if isempty(obj.logFiles)
                obj.logFiles = {[obj.ardupilotPath, 'log/newLog/log.mat']};
        ...
        function log = getLog(obj, l, p)
            ...
            obj.vars.logName = sprintf('%2d - %s', l, logFile{end});
        ...
        function plotCurve(obj, time, vect, legendStr, style, width, smoothFactor)
            ...
            args = {'DisplayName', obj.getLegend(legendStr), 'LineWidth', width}; 
            if exist('style', 'var') && ~isempty(style); args = [args, 'LineStyle', style]; end
            [t, x] = obj.trim(time, vect);
            if exist('smoothFactor', 'var'); x = obj.smooth(x, smoothFactor); end
            plot(t, x, args{:}); hold on
        end

        function legendStr = getLegend(obj, legendStr)
            if obj.overplotFCUs && length(obj.fcuList) > 1
                legendStr = sprintf('%s (fcu %d)', legendStr, obj.vars.fcu);
            end
        end
